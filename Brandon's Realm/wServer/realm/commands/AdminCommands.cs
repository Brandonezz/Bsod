...
namespace wServer.realm.commands
{
    ...
    internal class VisitCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (args.Length < 1)
            {
                player.SendHelp("Usage: /visit [Owner] <playername>");
                return false;
            }
            else if (String.Equals(player.Name.ToLower(), args[0].ToLower()))
            {
                player.SendInfo("You have autism if you tried to visit yourself.");
                return false;
            }
    ...
    internal class HostCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
                ...
                foreach (KeyValuePair<int, World> w in player.Manager.Worlds)
                {
                    ...
                    if (w.Key != 0)
                    {
                        world.BroadcastPacket(new TextPacket
                        {
                            ...
                            BubbleTime = 0,
                            Text =
                                "One of the staff members are going to host a fun arena! Come to nexus if you would like to participate in this!"
                        }, null);
                    }
                }
            }
            ...
            {
                player.SendError("Big no no announcement idiot");
                return false;
            }
...
internal class TossEffCommand : Command
    ...
    internal class SpawnCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            if (args.Length > 0 && int.TryParse(args[0], out num)) //multi
            {
                ...
                if (name == "vault chest")
                {
                    player.SendInfo("You cannot spawn that!");
                    return false;
                }
    ...
    internal class Max : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            try
            {
                ...
                player.Client.Save();
                player.UpdateCount++;
                player.SendInfo("Wow, What a loser you had to /max haha whatever here you go!");
            }
            catch
            {
                player.SendError("You had issues maxing idiot.");
                return false;
            }
            return true;
        }
    ...
    {
        public ChangePetSkinCommand() : base("cpskin", 3) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
    ...
    internal class ListCommands : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
           if (player.Client.Account.Rank > 2)
            {
            player.SendInfo("Commands : Teleport, ListCommands, Nexus, Where");
            }
